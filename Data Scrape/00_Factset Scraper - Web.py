...
# How To: Set the Start and End Dates to the Start of Current Year to Current Day Below.
# Input:  Start/End Dates (No leading 0s), Path to save PDFs, Additional/Tickers.xlsx, Email
...
def download_ticker_pdf(ticker):
    try:
        ...
        # Step 2: Click the checkbox to select all relevant documents
        ...
        ActionChains(driver).move_to_element(container).pause(1).click().perform()
        print("[SUCCESS] Document checkbox selected.")
        time.sleep(1)

        # Step 3: Click the correct download icon (full screen or compact)
        print("[INFO] Attempting to trigger download...")
        try:
            # Try full-screen mode download button
            full_download = WebDriverWait(driver, 3).until(
                EC.element_to_be_clickable((By.XPATH, "//*[@id='ss-download']/div/div[1]"))
            )
            driver.execute_script("arguments[0].click();", full_download)
            print("[SUCCESS] Download clicked (full screen).")
        except:
            # Fallback: compact mode via second wrench icon
            gear_icons = WebDriverWait(driver, 5).until(
                EC.presence_of_all_elements_located((By.XPATH, "//div[contains(@class, 'tools_DISEmWcm')]"))
            )
            if len(gear_icons) < 2:
                raise Exception("[ERROR] Could not find the correct wrench icon.")
        
            driver.execute_script("arguments[0].click();", gear_icons[1])
            print("[SUCCESS] Gear menu opened.")
            time.sleep(1)
        
            download_options = WebDriverWait(driver, 5).until(
                EC.presence_of_all_elements_located((By.XPATH, "//div[contains(text(), 'Download')]"))
            )
            for option in download_options:
                if option.is_displayed() and option.text.strip().lower() == "download":
                    driver.execute_script("arguments[0].click();", option)
                    print("[SUCCESS] Download clicked from compact menu.")
                    break
            time.sleep(1)
    
            # Step 4: Select "Save All Documents to One File" option (2nd radio button)
            radio_buttons = driver.find_elements(By.CSS_SELECTOR, "div.tfv-radio-sprites.icon_O53UeJ2D.radio-sprite_xMiOmUkF_light")
            if len(radio_buttons) >= 2:
                target_radio = radio_buttons[1]
                driver.execute_script("arguments[0].scrollIntoView(true);", target_radio)
                driver.execute_script("arguments[0].click();", target_radio)
                print("[SUCCESS] Selected 'Save all documents to one file'.")
                time.sleep(0.5)
    
            # Step 5: Click the final confirmation download button
            ...
            # Step 6: Wait for file download to complete and move it to destination folder
            print("[INFO] Waiting for downloaded file to appear...")
            download_dir = os.path.join(os.path.expanduser("~"), "Downloads")
            base_directory = r'S:\\Strategy Research\\Transcripts\\Data\\Raw Factset PDF'
            short_ticker = ticker.split('-')[0]  # Only use prefix before '-' for folder
            ...
            while time.time() - start_time < timeout:
                ...
                pdfs = [f for f in files if f.lower().endswith(".pdf") and not f.endswith(".crdownload")
                        and ("smart search" in f.lower() or "transcript" in f.lower())]
                if pdfs:
            ...
            if filename:
                ...
                print("[WARNING] Downloaded file not found within timeout.")

    except Exception as e:
        print(f"[FATAL] Download failed for {ticker}: {e}")
